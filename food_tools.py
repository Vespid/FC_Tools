...
def get_api_data():
    res=requests.get("https://data.import.io/extractor/4f9df99a-9114-4aa9-9730-14a365f17abc/json/latest?_apikey=65aa0274dc554730aa3ebd1cea2201658f433d3fb24785b14411dd47d97e9394c254b88a54638ff9c5c1a9edeeb073a730cc739dc42e1ac854107603b32f40341b1ec0d81aeb010ea13153ba9d1d51da")
    res.raise_for_status()
    ext_data=json.loads(res.text)
    data_list=ext_data["result"]['extractorData']['data'][0]["group"]
    
    Pirates=[]; Percent=[]; Payout=[]; daqPct=[]
    for x in data_list:
        Pirates.append(x["pirate"][0]["text"])
        Percent.append(realodds[int(x["opening_odds"][0]["text"][:-2])])
        Payout.append(int(x["current_odds"][0]["text"][:-2]))
        daqPct.append(.01*float(x["win_pct"][0]["text"][:-2]))
    roundData=[Pirates[0:4],Pirates[4:8],Pirates[8:12],Pirates[12:16],Pirates[16:20]]
    
    OddsData={}; PayoutData={}; daqData={}
    for x in range(len(Pirates)):
        OddsData[Pirates[x]]=Percent[x]
        PayoutData[Pirates[x]]=Payout[x]
        daqData[Pirates[x]]=daqPct[x]
