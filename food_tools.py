...
AORO_odds2={2:0.5226,
        3:0.2848437845541112,
        4:0.2225406290611273,
        5:0.1837640357906193,
        6:0.1571531212591245,
        7:0.13768374350215046,
        8:0.122779419318105,
        9:0.11097795321995477,
        10:0.10138571864882376,
        11:0.09342473185818319,
        12:0.08670402817511165,
        13:0.044,
        }
...
def get_todays_data():
    ...
    return roundData, OddsData, PayoutData, OpenOddsData
...
def today_AORO_bets(max_bet, risk):
    Arenas,o,Payouts,oo=get_todays_data()
...
def newodd_bets2(rnd, max_bet, risk):
    Arenas,o,Payouts,oo,r,fa,w,f,alg,Odds=load_all_data(rnd)
    ...
    for y in range(len(Arenas)):
        open_odds=[]; summation=0
        for x in Arenas[y]:
            open_odds.append(AORO_odds2[oo[x]])
        pp=Arenas[y]
        zipfile=[A for B, A in sorted(zip(open_odds,pp))]
        open_odds.sort()
        for x in range(len(open_odds)):
...
def test_newodd2(risks,rounds,max_bet):
    for risk in risks:
        total_win=[]; bets=0; total_TER=[]
        for rnd in rounds:
            win_file=open("fcwin_data.pickle","rb")
            ...
            win_file.close()
            win_data=wind[rnd]
            bet_today,TER=newodd_bets2(rnd, max_bet, risk)
            win=calc_winnings(bet_today,win_data)
            total_win.append(win)
            total_TER.append(TER)
            bets+=10
